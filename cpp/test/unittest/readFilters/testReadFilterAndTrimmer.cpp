...
{
    echidna::caller::params::Filters filterParams;
    ...
    auto emptyIntvlRead = std::make_shared< echidna::io::Read >( readParams, refSequence );
    ...
    auto emptyIntvlReadZeroAlignedLength = std::make_shared< echidna::io::Read >( readParams, refSequence );
...
BOOST_AUTO_TEST_CASE( testFilterFail )
{
    ...
    echidna::io::ReadFilterAndTrimmer rft( filterParams );
    auto notProperPairRead = std::make_shared< echidna::io::Read >( "WHY", "NOT", "test", Cigar( "3M" ), 0, 101, 0, 100,
    ...
    echidna::io::ReadFilterAndTrimmer rft_duplicates( filterParams );
    auto passesDuplicateFilterRead = std::make_shared< echidna::io::Read >(
    ...
    auto failsDuplicateFilterRead = std::make_shared< echidna::io::Read >(
...
BOOST_AUTO_TEST_CASE( testOverlapTrimming )
{
    ...
    auto refSequence = std::make_shared< echidna::utils::ReferenceSequence >(
        echidna::caller::Region( "1", -3, -3 + refSequenceStr.size() ), refSequenceStr );

    echidna::io::ReadFilterAndTrimmer rftTrimOverlap( filterParams );
    auto overlappingRead = std::make_shared< echidna::io::Read >( "WHYWHYWHYA",  // seq
                                                                  "NOTNOTNOTB",  // qual
                                                                  "test",        // readGroupId
...
BOOST_AUTO_TEST_CASE( testShortReadTrimming )
{
    ...
    echidna::io::ReadFilterAndTrimmer rftTrimShortRead( filterParams );
    ...
    auto reverseShortRead = std::make_shared< echidna::io::Read >( "WHYWHYWHYA",  // seq
                                                                   "NOTNOTNOTB",  // qual
                                                                   "test",        // readGroupId
                                                                   Cigar( "10M" ), 0, startPos,
                                                                   BAM_FPROPER_PAIR + BAM_FMREVERSE,  // flag
    ...
    auto noReverseShortRead = std::make_shared< echidna::io::Read >( "WHYWHYWHYA",  // seq
                                                                     "NOTNOTNOTB",  // qual
                                                                     "test",        // readGroupId
                                                                     Cigar( "10M" ), 0, startPos,
