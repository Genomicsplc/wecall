...
BOOST_AUTO_TEST_CASE( testWithEmptySetOfScores )
{
    ...
    auto bestScores = echidna::utils::indiciesWithHighestValues( scores, 100 );
...
BOOST_AUTO_TEST_CASE( testGetsAllIndiciesIfTotalAllowedIsHigh )
{
    ...
    auto bestScores = echidna::utils::indiciesWithHighestValues( scores, 2 );
...
{
    std::vector< double > scores = {1.0, 2.0};
    auto bestScores = echidna::utils::indiciesWithHighestValues( scores, 1 );
...
BOOST_AUTO_TEST_CASE( testShouldNotPickOutLowScores )
{
    ...
    const auto bestScores = echidna::utils::indiciesWithHighestValues( scores, scores.size(), 1.0, 1.0e8 );
    BOOST_CHECK_EQUAL( bestScores.size(), 2 );
